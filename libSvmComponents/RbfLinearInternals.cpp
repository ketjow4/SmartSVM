...
geneticComponents::Population<SvmCustomKernelChromosome> CusomKernelGenerationRbfLinearSequential::createPopulation(uint32_t populationSize)
{
	//populationSize = 20;
	if (populationSize == 0)
	{
	...
	std::generate(population.begin(), population.end(), [&]
	{
		...
		/*if(m_gammaSingle == -1)
		{
			m_C = { 0.1,1,10,100,1000 };
			auto cValueRandom = std::uniform_int_distribution<int>(0, static_cast<int>(m_C.size()) - 1);
			auto cValue = m_rngEngine->getRandom(cValueRandom); 
			return SvmCustomKernelChromosome(std::move(chromosomeDataset), m_C[cValue]);
		}
		else*/
		{
...
geneticComponents::Population<SvmCustomKernelChromosome> CrossoverCompensationRbfLinear::compensate(
...
void EducationOfTrainingSetRbfLinear::educatePopulation(geneticComponents::Population<SvmCustomKernelChromosome>& population,
...
bool EducationOfTrainingSetRbfLinear::replacementCondition(const Gene& supportVectorPoolElement,
...
void EducationOfTrainingSetRbfLinear::educate(SvmCustomKernelChromosome& individual,
...
std::vector<Gene> EducationOfTrainingSetRbfLinear::setDifference(const std::unordered_set<uint64_t>& svPool,
...
std::unordered_set<uint64_t> EducationOfTrainingSetRbfLinear::findSupportVectors(const geneticComponents::Parents<SvmCustomKernelChromosome>& parents,
                                                                                 const dataset::Dataset<std::vector<float>, float>& trainingSet) const
{
...
const std::vector<Gene>& SupportVectorPoolRbfLinear::getSupportVectorPool() const
{
	return m_supportVectorPool;
}

const std::unordered_set<uint64_t>& SupportVectorPoolRbfLinear::getSupportVectorIds() const
{
	return m_supportVectorIds;
}

void SupportVectorPoolRbfLinear::updateSupportVectorPool(const geneticComponents::Population<SvmCustomKernelChromosome>& population,
...
unsigned int SupportVectorPoolRbfLinear::findPositionOfSupprotVector(const dataset::Dataset<std::vector<float>, float>& individualDataset,
...
void SupportVectorPoolRbfLinear::addSupportVectors(const SvmCustomKernelChromosome& chromosome,
                                                   const dataset::Dataset<std::vector<float>, float>& trainingSet)
{
		...
		for (auto i = 0; i < supportVectors.size(); i++)
		{
			//if (scores[i] > 0.95)
			{
				...
				if (m_supportVectorIds.emplace(dataset[positionInDataset].id).second && (dataset[positionInDataset].gamma == m_currentGamma
					/*|| classifier->getKernelType() == phd::svm::KernelTypes::RBF_LINEAR && dataset[positionInDataset].gamma == -1)*/))
				{
...
bool emplacementCondition_rbfLinear(const Gene& drawnSupportVector,
...
std::vector<SvmCustomKernelChromosome> SuperIndividualsCreationRbfLinear::generate(unsigned int populationSize,
                                                                                   const std::vector<Gene>& supportVectorPool,
                                                                                   unsigned int numberOfClassExamples)
{
	...
	std::generate(population.begin(), population.end(), [&]
	{
		...
		}))
		{
			...
			if (emplacementCondition_rbfLinear(drawnSupportVector, classCount, numberOfClassExamples, trainingSet, m_imbalancedOrOneClass))
			{
...
Population<SvmCustomKernelChromosome> SuperIndividualsCreationRbfLinear::createPopulation(uint32_t populationSize,
                                                                                          const std::vector<Gene>& supportVectorPool,
                                                                                          unsigned int numberOfClassExamples)
{
	if (populationSize == 0)
...
bool MemeticTrainingSetAdaptationRbfLinear::getIsModeLocal() const
{
	return m_isLocalMode;
}

unsigned int MemeticTrainingSetAdaptationRbfLinear::getNumberOfClassExamples() const
{
	return m_numberOfClassExamples;
}

void MemeticTrainingSetAdaptationRbfLinear::resetToInitial(unsigned int numberOfClassExamples)
{
...
void MemeticTrainingSetAdaptationRbfLinear::setFrozenSetSize(unsigned int size)
{
	m_frozenOnesSize = size;
}

MemeticTrainingSetAdaptationRbfLinear::MemeticTrainingSetAdaptationRbfLinear(bool isLocalMode,
                                                                             unsigned int numberOfClassExamples,
...
void MemeticTrainingSetAdaptationRbfLinear::validate() const
{
...
void MemeticTrainingSetAdaptationRbfLinear::growSizeOfTraningSet(double bestOneFitness, platform::Percent percentOfSupportVectors)
{
...
bool MemeticTrainingSetAdaptationRbfLinear::adaptationCondition(double deltaIteration,
                                                                double deltaMode,
...
void MemeticTrainingSetAdaptationRbfLinear::adapt(geneticComponents::Population<SvmCustomKernelChromosome>& population)
{
...
std::vector<unsigned int> CompensationInformationRbfLinear::generate(const std::vector<Parents<SvmCustomKernelChromosome>>& parents,
                                                                     unsigned int numberOfClassExamples) const
{
	...
	std::transform(parents.begin(), parents.end(), compensationInfo.begin(), [&, this](const auto& parentsPair)
	{
		...
		//std::uniform_int_distribution<int> sizeOfChild(static_cast<int>(parentsMax), static_cast<int>(m_numberOfClasses * numberOfClassExamples));
		auto min = std::min(static_cast<int>(parentsMax), static_cast<int>(m_numberOfClasses * numberOfClassExamples));
		auto max = std::max(static_cast<int>(parentsMax), static_cast<int>(m_numberOfClasses * numberOfClassExamples));
...
void MutationRbfLinear::mutatePopulation(geneticComponents::Population<SvmCustomKernelChromosome>& population)
{
	if (population.empty())
	{
		throw geneticComponents::PopulationIsEmptyException();
...
std::unordered_set<uint64_t> MutationRbfLinear::setDifference(const std::vector<Gene>& set,
...
void MutationRbfLinear::calculateNumberOfPossibleExchanges(SvmCustomKernelChromosome& chromosome,
...
std::vector<Gene> MutationRbfLinear::findReplacement(const std::unordered_set<uint64_t>& deleted,
                                                     ...
                                                     SvmCustomKernelChromosome& chromosome) const
{
	...
	{
		//if (chromosome.getClassifier()->getKernelType() == phd::svm::KernelTypes::RBF_LINEAR)
		//{
		//	std::bernoulli_distribution kernel_change(m_mutationProbability.m_percentValue);
		//	if (m_rngEngine->getRandom(kernel_change))
		//	{
		//		//indicates change of kernel to linear
		//		dataset[positionsToReplace[i]].gamma = -1;
		//		continue;
		//	}
		//}
...
inline void MutationRbfLinear::getPositionsOfMutation(SvmCustomKernelChromosome& chromosome,
...
void MutationRbfLinear::mutateChromosome(SvmCustomKernelChromosome& chromosome)
{
	std::unordered_set<uint64_t> deleted;
