...
dataset::Dataset<std::vector<float>, float> joinSets(const dataset::Dataset<std::vector<float>, float>& tr,
                                                     const dataset::Dataset<std::vector<float>, float>& val)
{
	...
	{
		joined_tr_val.addSample(tr.getSample(i), tr.getLabel(i));
	}

	for (auto i = 0u; i < val.size(); ++i)
	{
		joined_tr_val.addSample(val.getSample(i), val.getLabel(i));
	}

	return joined_tr_val;
}

SvmHelper::SvmHelper(const SvmWokrflowConfiguration& config,
...
void SvmHelper::logAllModels(AllModelsLogger& logger)
{
...
void SvmHelper::VisualizeWholePopulation(unsigned& /*numberOfRun*/)
{
...
std::shared_ptr<phd::svm::ISvm> SvmHelper::run()
{
...
svmComponents::SvmTrainingSetChromosome SvmHelper::getBestOne() const
{
	return m_pop.getBestOne().getTraining();
}

void SvmHelper::addVectorsToPopulation(std::vector<DatasetVector>& SVs)
{
...
void SvmHelper::init()
{
	try
	{
		...
		//
		//Used for some stability experiments where we start from the same pop
		//
...
geneticComponents::Population<svmComponents::SvmKernelChromosome> SvmHelper::getKernelPopulation()
{
...
geneticComponents::Population<svmComponents::SvmFeatureSetMemeticChromosome> SvmHelper::getFeaturesPopulation()
{
	if(m_useFeatureSelection)
	{
...
geneticComponents::Population<svmComponents::SvmTrainingSetChromosome> SvmHelper::getTrainingSetPopulation()
{
...
void SvmHelper::setKernelPopulation(geneticComponents::Population<svmComponents::SvmKernelChromosome>& population,
...
void SvmHelper::setTrainingPopulation(geneticComponents::Population<svmComponents::SvmTrainingSetChromosome>& population,
                                      geneticComponents::Population<svmComponents::SvmSimultaneousChromosome>& newPop)
{
	int i = 0;
	for (const auto& individual : population)
...
void SvmHelper::setFeaturesPopulation(geneticComponents::Population<svmComponents::SvmFeatureSetMemeticChromosome>& population,
                                      geneticComponents::Population<svmComponents::SvmSimultaneousChromosome>& newPop)
{
	int i = 0;
	for (const auto& individual : population)
...
void SvmHelper::updateMetricDistance()
{
...
void SvmHelper::fixKernelPop2(const geneticComponents::Population<svmComponents::SvmSimultaneousChromosome>& currentPop,
                              geneticComponents::Population<svmComponents::SvmSimultaneousChromosome>& newPop)
{
...
void SvmHelper::performEvolution()
{
	try
	{
...
void SvmHelper::train(Population<SvmSimultaneousChromosome>& pop)
{
...
void SvmHelper::evaluate(Population<SvmSimultaneousChromosome>& pop)
{
